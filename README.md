...
- Bosque masivo de 52.000 árboles Quaternius: una retícula irregular de 54 m garantiza cobertura sobre 7.755 celdas verdes y el resto del presupuesto forma reservas profundas y acompaña rutas. Los ejemplares azul petróleo forman el Bosque Tenebroso y los árboles rojos quedan por debajo del 1%. En la cordillera, los pinos pasan gradualmente de verdes a mixtos y finalmente blancos según la altura.
...
- Diario de 200 descubrimientos: una barra superior muestra el progreso, `L` abre la lista completa y `E` registra la zona cercana y autoguarda. Al elegir una entrada, el mapa se abre y señala su destino; amanecer y atardecer también cuentan como descubrimientos especiales.
...
| Equipar cuchillo / hacha / antorcha / brújula | `1` / `2` / `3` / `4` | — |
| Atacar con el objeto equipado | Clic izquierdo | Botón superior |
| Explorar / montar / desmontar | `E` (se prioriza una zona cercana) | Botón izquierdo |
...
| Abrir diario de 200 descubrimientos | `L` | — |
...
El *Survival Pack - Sept 2020* está disponible en `assets/quaternius/Survival Pack - Sept 2020`. Cuchillo, hacha, antorcha con luz y brújula se construyen directamente desde sus OBJ al pulsar `1–4`; el juego empieza con las manos vacías y solo el objeto seleccionado aparece unido al hueso `Fist.R`. La *Universal Animation Library 2* está en `assets/animations` para acciones futuras; la locomoción actual usa las animaciones del propio personaje, sincronizadas con su desplazamiento.
