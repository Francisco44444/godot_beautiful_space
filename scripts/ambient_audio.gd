...
func _process(delta: float) -> void:
	...
	var special_weight := clampf(maxf(snow_music_weight, desert_music_weight), 0.0, 1.0)
	...
	snow_music.volume_db = move_toward(snow_music.volume_db, _weighted_music_db(music_base_db, snow_music_weight), fade_speed * delta)
	desert_music.volume_db = move_toward(desert_music.volume_db, _weighted_music_db(music_base_db, desert_music_weight), fade_speed * delta)
	wind.volume_db = move_toward(wind.volume_db, wind_volume_db + riding_amount * 4.5, fade_speed * delta)
	# En las Dunas Doradas no hay pájaros de valle. Se desvanecen antes de cruzar
	# el límite y en el interior solo queda Ashes (más el viento físico al galopar).
	var bird_desert_mute := smoothstep(0.08, 0.62, desert_music_weight)
	var bird_target := lerpf(birds_volume_db - riding_amount * 3.0, -60.0, bird_desert_mute)
...
func _update_biome_weights() -> void:
	...
	desert_music_weight = 1.0 - smoothstep(0.78, 1.04, desert_radius)
	...
	snow_music_weight = smoothstep(0.10, 0.52, snow_probability) * (1.0 - desert_music_weight)

	var next_zone := "valley"
	if desert_music_weight >= 0.5:
		next_zone = "desert"
