...
func _control_pixel(point: Vector2, height: float) -> Color:
	...
	# Sendero nominal de 5-13 m: con el vértice low-poly de 9,76 m produce una
	# cinta jugable estrecha y deja que la vegetación abrace ambos bordes.
	if road_distance < 13.0 and height > 0.5:
		var road_strength := 1.0 - smoothstep(2.5, 13.0, road_distance)
